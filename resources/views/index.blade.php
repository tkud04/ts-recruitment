  ...
  <div class="container">
    <?php
     for($i = 0; $i < count($jobs); $i++){
      ...
      $job = $jobs[$i];
    ?>
    <div class="single-jobs{{$class}}"> <i class="fa fa-{{$job['icon']}}"></i>
      <div class="job-heading">
        <h3>{{$job['title']}}</h3>
        <p>{{$job['description']}}</p>
      </div>
      <div class="our-location color{{$i + 1}}"> <span class="fa fa-map-marker" aria-hidden="true"></span>
        <div class="location-content">
          <h3>{{$job['location']}}</h3>
          <span>{{$job['availability']}}</span> </div>
      </div>
    </div>
